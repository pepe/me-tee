...
describe Icon do
  ...
  it "is filled with right attributes when initialized" do
    icon = Icon.new(:label => 'cook', :filename => 'cook.gif', :type => 'hobby')
    icon.label.should == 'cook'
  end
  ...
  it "is then retrieved by key like 'hobby-cook'" do
    icon = Icon.create(:label => 'cook', :filename => 'cook.gif', :type => 'hobby')
  ...
  context "type scopes" do
    ...
    it "finds face icons " do
      icon = Icon.create(:label => 'cook', :filename => 'cook.gif', :type => 'face')
      Icon.faces.should == [icon]
    end
    it "finds job icons " do
      icon = Icon.create(:label => 'cook', :filename => 'cook.gif', :type => 'job')
  ...
  context 'random' do
    it 'should get random icon for type' do
      %(swim voley read).each do |icon|
        Icon.create(:label => icon, :type => 'hobby', :filename => "#{icon}.gif")
      end
      Icon.create(:label => 'angry', :type => 'face', :filename => "angry.gif")
